...
/**
 *  Parses a group membership definition file. We assume that the file will have the
 *  following fields:<br/>
 *
 *  <ul>
 *    <li> course_id</li>
 ...
 *  </ul>
 *  <br/>
 *  These fields should be delimited by a pipe. Comments may begin with a # as
 *  the first character in the line.<br/>
...
public class GroupMembershipParser {
	...
	private final static Pattern commentPattern = Pattern.compile("^#.*$");//Starts with a #, anything to the end
		...
		while (aLine != null) {
			...
			if ((aLine.length() == 0)  || commentMatcher.matches()) {
				...
				if (!(courseSet.add(aRecord) && groupSet.add(aRecord))) {
					logger.info("Discarded duplicate record " + aRecord.getExternalCourseKey() + "|" + aRecord.getExternalGroupKey());
				}
			} else {
	 ...
	 * @return           A Set of GroupMembershipRecords whose courseId and external groupId match
	 *      the provided values, empty if none exist.
	 */
	 ...
	 *
	 * @param  courseId  The course id the GroupMembershipRecord should have
	 * @return           A Set of GroupMembershipRecords whose courseId matchse the provided
	 *      value, which may have a zero-length
	 */
